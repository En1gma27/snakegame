...
public class GamePanel extends JPanel implements ActionListener{
	...
	public void checkCollisions() {
		// kiem tra dau co cham voi than ko
		for(int i = bodyParts;i>0;i--) {
			if((x[0] == x[i])&& (y[0] == y[i])) {
				running = false;
			}
		}
		//kiem tra xem dau co cham leftborder ko
		if(x[0] < 0) {
			running = false;
		}
		//kiem tra xem dau co cham righttborder ko
		if(x[0] > SCREEN_WIDTH) {
			running = false;
		}
		//kiem tra xem dau co cham bottomborder ko 
		if(y[0] < 0) {
			running = false;
		}
		//kiem tra xem dau co cham topborder ko ko
